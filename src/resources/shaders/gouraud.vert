...
void main()
{
    vPos = vec4(modelViewMatrix * vec4(position, 1)).xyz;
    ...
    totalLight += calcPointLight(pointLight, vPos, vNorm);

    if ( material.useColour == 1 )
        vColor = vec4(material.colour, 1);
    else
        vColor = texture(texture_sampler, texCoord);
